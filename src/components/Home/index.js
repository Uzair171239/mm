...
function Home() {
  ...
  useEffect(() => {
    if (country.id !== 0 && country !== "undefined") {
      axios
        .get(`http://localhost:3001/api/products/${country.id}`)
        .then(({ data }) => {
          set_initData(data);
          setHotDeals(
  ...
  return (
    <div className="relative">
      {country.id === 0 && (
        <div className="flex justify-center pt-10 w-full h-screen absolute z-50 bg-white/5 backdrop-blur-sm">
      ...
      {/* Hot Deals *************************/}
      {filteredData[0] ? (
        <AllProduct data={filteredData} country={country} />
      ) : (
        <>
