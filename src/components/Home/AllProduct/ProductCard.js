...
function ProductCard({ product }) {
  const {price, offer_percentage, image, title, old_price, with_vat, view_vat } = product;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col py-2">
        ...
        <p className="text-gray-900 hover:text-red-500 text-sm md:text-base">{title.substring(0, 45)}...</p>
        ...
        <p className="text-gray-900 text-sm md:text-base">{view_vat} AED - VAT included</p>
      </div>
    </div>
  );
}
